...
the embedded Blocknet daemon with: --api --daemon")]
struct Cli {
...
fn try_spawn_embedded_daemon() -> Result<PathBuf, String> {
...
async fn main() -> color_eyre::Result<()> {
    ...
    if !cookie_path.is_file() && cli.cookie.is_none() && is_local_host(&cli.host) && cli.port == 8332 {
    ...
    let api = if launched_embedded {
        match wait_for_daemon(&base_url, &cookie_path, 30).await {
            Ok(api) => api,
            Err(e) => {
                eprintln!("error: {e}");
        ...
        if !cookie_path.is_file() {
            ...
            eprintln!("  - port is 8332");
            eprintln!("  - no custom --cookie is provided");
            eprintln!();
            eprintln!("Otherwise start Blocknet manually with --api --daemon.");
            std::process::exit(1);
        }
        ...
        if let Err(e) = api.get_status().await {
            if cli.cookie.is_none() && is_local_host(&cli.host) && cli.port == 8332 {
                match try_spawn_embedded_daemon() {
                    Ok(path) => {
                        eprintln!("started embedded blocknet daemon: {}", path.display());
                        match wait_for_daemon(&base_url, &cookie_path, 30).await {
